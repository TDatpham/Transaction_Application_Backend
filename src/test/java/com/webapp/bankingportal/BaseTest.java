...
public abstract class BaseTest {
    ...
    protected static String getRandomCountryCode() {
        String[] supportedRegions = phoneNumberUtil.getSupportedRegions().toArray(new String[0]);
        int index = faker.number().numberBetween(0, supportedRegions.length - 1);
        return supportedRegions[index];
    }
    ...
    protected static String getRandomPhoneNumber(String region) {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            try {
                Phonenumber.PhoneNumber phoneNumber = phoneNumberUtil.parse(randomPhoneNumber, region);
    ...
    protected Map<String, String> createAndLoginUser() throws Exception {
        ...
        LoginRequest loginRequest = new LoginRequest(accountNumber, user.getPassword());

        String loginResult = mockMvc.perform(MockMvcRequestBuilders
        ...
        String token = JsonPath.read(loginResult, "$.token");
    ...
    protected Map<String, String> createAndLoginUserWithPin() throws Exception {
        Map<String, String> userDetails = createAndLoginUser();
        String accountNumber = userDetails.get("accountNumber");
        String password = userDetails.get("password");

        PinRequest pinRequest = new PinRequest(accountNumber, getRandomPin(), password);
    ...
    protected Map<String, String> createAndLoginUserWithInitialBalance(double amount) throws Exception {
        Map<String, String> userDetails = createAndLoginUserWithPin();
        AmountRequest amountRequest = new AmountRequest(userDetails.get("accountNumber"), userDetails.get("pin"), amount);
    ...
    protected Map<String, String> createAccount() {
        ...
        String accountNumber = accountService.createAccount(user).getAccountNumber();
        accountDetails.put("accountNumber", accountNumber);
        return accountDetails;
    }

    protected Map<String, String> createAccountWithPin(PasswordEncoder passwordEncoder,
            UserRepository userRepository, AccountService accountService) {
        Map<String, String> accountDetails = createAccount();
    ...
    protected Map<String, String> createAccountWithInitialBalance(double amount) {
        Map<String, String> accountDetails = createAccountWithPin(passwordEncoder, userRepository, accountService);
